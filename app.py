...
if uploaded_file is not None and st.button("🚀 Обработать файл"):
    with st.spinner("Идёт обработка..."):
        ...
        # Ключевое исправление: приводим столбец M к строке, чтобы потом записывать пустые строки
        df["Мое предложение (самовывоз)"] = df["Мое предложение (самовывоз)"].astype(str)
        ...
        for idx, row in df.iterrows():
            if pd.notna(row.get("Цена_из_справочника")):
                ...
                # Очищаем столбец M (самовывоз) — делаем пустым
                df.at[idx, "Мое предложение (самовывоз)"] = ""
        ...
        # Дополнительная очистка столбца M от "nan" и None (на всякий случай)
        df["Мое предложение (самовывоз)"] = df["Мое предложение (самовывоз)"].astype(str).replace("nan", "").replace("None", "")
