...
if uploaded_file is not None and st.button("🚀 Обработать файл"):
    with st.spinner("Идёт обработка..."):
        ...
        # Столбец M оставляем как есть (будем заполнять пустыми позже)
        ...
        # Заменяем NaN на пустые строки во всех колонках
        df = df.fillna("")

        # Принудительно приводим столбец M к строке (на случай, если там что-то осталось)
        df["Мое предложение (самовывоз)"] = df["Мое предложение (самовывоз)"].astype(str).replace("nan", "")
